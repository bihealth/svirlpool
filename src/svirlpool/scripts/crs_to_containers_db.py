...
# def annotated_readname_to_readname_and_sampleID(annotated_readname:str) -> tuple[str,int]:
#     x = annotated_readname.split('.')
#     return x[0],int(x[1])

# def sum_dels_per_read(cr:datatypes.CandidateRegion,readnames:list[str]) -> tuple[list[str],dict[str,int]]:
#     sum_dict:dict[str,int] = {readname:0 for readname in readnames}
#     for signal in cr.sv_signals:
#         if signal.sv_type == 1:
#             sum_dict[signal.aug_readname()] += int(signal.size)
#     return readnames,sum_dict

# def sum_ins_per_read(cr:datatypes.CandidateRegion,readnames:list[str]) -> tuple[list[str],dict[str,int]]:
#     sum_dict:dict[str,int] = {readname:0 for readname in readnames}
#     for signal in cr.sv_signals:
#         if signal.sv_type == 0:
#             sum_dict[signal.aug_readname()] += int(signal.size)
...
# def n_bnds_per_read(cr:datatypes.CandidateRegion,readnames:list[str]) -> tuple[list[str],dict[str,int]]:
#     sum_dict:dict[str,int] = {readname:0 for readname in readnames}
#     for signal in cr.sv_signals:
#         if signal.sv_type >= 3:
#             sum_dict[signal.aug_readname()] += 100
#     return readnames,sum_dict
...
#     readnames = sorted(set([signal.aug_readname() for signal in cr.sv_signals]))
...
#     matrix = np.array([list(sums_inss.values()),list(sums_dels.values()),list(sums_bnds.values())]).T
...
#     df['cluster'] = labels
#     fig = px.scatter(df,x='deletions',y='insertions',color='cluster',title=f"crID: {cr.crID}, n_clusters: {len(set(labels))}")
#     fig.show()

# def find_initial_clustering(matrix:np.ndarray,max_clusters:int=10) -> dict:
#     labels = np.zeros(len(matrix),dtype=int)
#     cluster_evaluation = evaluate_clustering(X=matrix,labels=labels)
#     value = sum([np.max(ordered_differences(matrix[:,i])) for i in range(matrix.shape[1])])
#     last_clustering = {'value':float(value),'n_clusters':1,'labels':labels.tolist(),'cluster_evaluation':cluster_evaluation.tolist()}
#     n_distinct_rows = len(set([tuple(row) for row in matrix]))
#     if n_distinct_rows < 3 or max_clusters < 2:
#         value = 0 if len(matrix) == 1 else sum([np.max(ordered_differences(matrix[:,i])) for i in range(matrix.shape[1])])
#         n_clusters = 1
#         labels = np.zeros(len(matrix),dtype=int)
#         return {'value':float(value),'n_clusters':int(n_clusters),'labels':labels.tolist(),'cluster_evaluation':cluster_evaluation.tolist()}
...
#         last_clustering = {'value':float(value),'n_clusters':int(n_clusters),'labels':np.array(clustering.labels_,dtype=int).tolist(),'cluster_evaluation':cluster_evaluation.tolist()}
#     return last_clustering

# def cr_to_measures_matrix(cr:datatypes.CandidateRegion) -> np.ndarray:
#     # returns a ndarray with columns: insertions, deletions, BNDs for each readname
#     readnames = sorted(set([signal.aug_readname() for signal in cr.sv_signals]))
#     _,sums_dels = sum_dels_per_read(cr=cr,readnames=readnames)
#     _,sums_inss = sum_ins_per_read(cr=cr,readnames=readnames)
#     readnames,sums_bnds = n_bnds_per_read(cr=cr,readnames=readnames)

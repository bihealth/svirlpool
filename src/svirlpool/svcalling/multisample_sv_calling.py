...
class SVcall:
    ...
    def to_vcf_line(
        ...
        if synthetic_coverage and not legacy_force_wildtype:
            # Mark records whose depth-derived fields come from the fabricated
            # uniform coverage of --skip-covtrees rather than from real tracks.
            info_line += ";SYNTHETIC_COV"
        ...
        for samplename in samplenames:
            ...
            if gt is None:
                ...
                elif coverage == 0 or synthetic_coverage:
                    format_content.append(
...
def genotype_of_sample(
    ...
    if len(alt_reads) == 0:
        ...
        if synthetic_coverage and not legacy_force_wildtype:
            # --skip-covtrees fabricates a uniform coverage track, so "covered"
            # here carries no information about the sample at all.  Refuse to
            # emit a confident reference call from a fabricated depth.
            log.warning(
                f"SVcalls_from_SVcomposite: coverage tracks are synthetic (--skip-covtrees); "
                f"refusing a homozygous-reference call for sample {samplename} at "
                f"{chrname}:{start}-{end}. Emitting a no-call ({NO_CALL_GENOTYPE})."
            )
            return create_no_call_genotype(samplename=samplename)
...
def generate_header(
    ...
    if synthetic_coverage:
        header.append(
            '##INFO=<ID=SYNTHETIC_COV,Number=0,Type=Flag,Description="Coverage tracks '
            "were fabricated by --skip-covtrees (uniform coverage), not measured from "
            'the sample; TC/DR/DV and GQ carry no information about this sample">'
        )
    header.append(
...
def write_svCalls_to_vcf(
    ...
    with open(tmp_result_unsorted.name, "w") as f:
        ...
        header = generate_header(
            ...
            synthetic_coverage=synthetic_coverage and not legacy_force_wildtype,
        )
        for line in header:
            print(line, file=f)
        # write SVcalls
...
def multisample_sv_calling(
    ...
    if skip_covtrees:
        log.warning(
            "--skip-covtrees: coverage tracks are fabricated (uniform coverage of 30) "
            "and carry no information about the samples. Homozygous-reference calls "
            "cannot be justified from them, so loci without alternate support are "
            "emitted as no-calls and every record is flagged SYNTHETIC_COV."
        )
        covtrees: dict[str, dict[str, IntervalTree]] = (

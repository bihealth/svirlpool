...
workdir = Path("/home/vinzenz/development/LRSV-detection/tests/data/consensus")
path_lamassemble_mat = Path(
    "/home/vinzenz/development/lamassemble/train/promethion.mat"
)
...
ref_deletion = reference[:4000] + reference[4500:]
ref_insertion = (
    reference[:5500] + util.generate_sequence(size=1000, seed=19) + reference[5500:]
)

reads_deletion_incr = [
    SeqRecord(
        id=f"read.deletion_incr.{incr}",
        name=f"read.deletion_incr.{incr}",
        description=f"len:{8000-incr}",
        seq=Seq(
            "".join(
                util.generate_read_from_rules(
                    rules=[(util.delete_interval, 4000, 4000 + incr)],
                    reference=reference,
                    refStart=1000,
                    refEnd=9000,
                )
    ...
    for incr in range(50, 551, 50)
]

ins_sequence = util.generate_sequence(size=500, seed=19)
reads_insertion_incr = [
    SeqRecord(
        id=f"read.insertion_incr.{incr}",
        name=f"read.insertion_incr.{incr}",
        description=f"len:{8000+incr}",
        seq=Seq(
            "".join(
                util.generate_read_from_rules(
                    rules=[(util.insertion, 4000, incr, ins_sequence[:incr])],
                    reference=reference,
                    refStart=1000,
                    refEnd=9000,
                )
...
dict_refsets = {"deletion": ref_deletion, "insertion": ref_insertion}

dict_readsets = {"deletion": [], "insertion": []}

region_on_ref = (2_000, 9_000)
readstarts = range(0, 1001, 100)
readends = range(2_200, 0, -200)

dict_reads = {}

for start, end in zip(readstarts, readends):
    for sv in dict_readsets.keys():
        dict_readsets[sv].append(
            SeqRecord(
                id=f"read.{sv}.{start}.-{end}",
                name=f"read.{sv}.{start}.-{end}",
                description=f"len:{len(dict_refsets[sv][start:-end])}",
                seq=Seq("".join(dict_refsets[sv][start:-end])),
            )
        )

dict_reads["deletion_incr"] = reads_deletion_incr
dict_reads["insertion_incr"] = reads_insertion_incr

dict_readsets["reads_deletion_incr"] = reads_deletion_incr
dict_readsets["reads_insertion_incr"] = reads_insertion_incr

# write reference to file
path_reference = tmp_dir_path / "reference.fasta"
reference_record = SeqRecord(
    seq=Seq("".join(reference)), id="ref", description="", name="ref"
)
SeqIO.write(reference_record, path_reference, "fasta")

# write reads to fasta file
path_reads = tmp_dir_path / "reads.fasta"
SeqIO.write(
    [r for sv in dict_readsets.keys() for r in dict_readsets[sv]], path_reads, "fasta"
)

# align all reads to the reference
path_alignments = tmp_dir_path / "reads.toRef.bam"
util.align_reads_with_minimap(
    reference=tmp_dir_path / "reference.fasta",
    reads=tmp_dir_path / "reads.fasta",
    bamout=tmp_dir_path / path_alignments,
    threads=3,
)

# %%

path_intervals_db = tmp_dir_path / "intervals.db"
path_reads_db = tmp_dir_path / "reads.db"
# calc all intervals for region_on_ref
crs_to_dbs.create_intervals_db(path_intervals_db=path_intervals_db, timeout=300.0)
crs_to_dbs.create_reads_db(path_reads_db=path_reads_db, timeout=300.0)
buffer_reads = []
buffer_intervals = []
for a in pysam.AlignmentFile(path_alignments):
    buffer_reads.append((a.query_name, 0, a.query_sequence, a.query_qualities))
    buffer_intervals.append(
        (
            ...
            *util.get_interval_on_read_in_region(
                a=a, start=region_on_ref[0], end=region_on_ref[1]
            ),
            0,
            *region_on_ref,
            0,
            ...
            max(a.reference_start, region_on_ref[0]),
            min(a.reference_end, region_on_ref[1]),
        )
    )
    crs_to_dbs.write_intervals_to_intervals_db(
        buffer_intervals=buffer_intervals,
        path_intervals_database=path_intervals_db,
        timeout=300.0,
    )
    crs_to_dbs.write_reads_to_reads_db(
        buffer_reads=buffer_reads, path_reads_database=path_reads_db, timeout=300.0
    )
# %%

cutreads, intervals = consensus_lib.trim_reads(
    mcrID=0, path_reads_db=path_reads_db, path_intervals_db=path_intervals_db
)

dict_cutreads = {sv: dict() for sv in dict_readsets.keys()}
for readID, seqRecord in cutreads.items():
    readsv = readID.split(".")[1]
    dict_cutreads[readsv][readID] = seqRecord

dict_intervals = {sv: [] for sv in dict_readsets.keys()}
for row in intervals:
    readsv = row[0].split(".")[1]
    dict_intervals[readsv].append(row)

reads_partitioning_threshold = 6.0
min_signal_size = 30
# create combination of reads to test with
for a, b in combinations(dict_readsets.keys(), 2):
    tmp_dir_ex = tempfile.TemporaryDirectory(prefix=str(tmp_dir_path / f"{a}.{b}."))
    reads = {**dict_cutreads[a], **dict_cutreads[b]}
    intervals = dict_intervals[a] + dict_intervals[b]
    reads = consensus.sort_reads_by_size(reads=reads, intervals=intervals)
    pool = {r.id: len(r.seq) for r in reads.values()}
    path_tmp_fastq = Path(tmp_dir_ex.name) / "reads.fastq"
    SeqIO.write(reads.values(), path_tmp_fastq, "fasta")
    dict_partitions = consensus.create_read_partitions(
        pool=pool,
        reads=reads,
        path_tmp_fastq=path_tmp_fastq,
    ...
    for i, (r0_id, read_ids) in enumerate(dict_partitions.items()):
        ...
        path_partition_reads = Path(tmp_dir.name) / f"reads.{i}.fastq"
        with open(path_partition_reads, "w") as f:
            SeqIO.write([reads[read_id] for read_id in read_ids], f, "fasta")
        path_consensus = Path(tmp_dir.name) / f"consensus.{i}.fasta"
        consensus_meta[i].append(path_consensus)
        # maybe use length for normalization of SV signal?
        consensus_length = consensus.make_consensus(
            longest_read_id=r0_id,
            dict_partitions=dict_partitions,
            reads=reads,
            name=f"consensus.{0}.{i}",
            threads=1,
            path_consensus=path_consensus,
            lamassemble_mat_path=path_lamassemble_mat,
            min_coverage=0.1,
        )
...
reads_deletion = [
    util.generate_read_from_rules(
        rules=[], reference=reference, refStart=readstarts[0], refEnd=readends[0]
    ),
    util.generate_read_from_rules(

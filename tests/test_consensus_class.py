...
def load_test_data(prefix:str) -> tuple[consensus_class.Consensus, Alignment]:
    ...
    with gzip_open(alignment_path, "rt") as f:
        ...
        alignment: Alignment = cattrs.structure(
            json.loads(alignment_data), Alignment
    )
...
def test_get_consensus_core_alignment_interval_on_reference_inv():
    ...
    assert abs(start - expected[1]) <= 100, f"start {start} vs expected {expected[1]}; consensus padding: {cons1.consensus_padding}"
    assert abs(end - expected[2]) <= 100, f"end {end} vs expected {expected[2]}; consensus padding: {cons1.consensus_padding}"
    # test 11.1
    ...
    assert abs(start2 - expected2[1]) <= 100, f"start {start2} vs expected {expected2[1]}; consensus padding: {cons2.consensus_padding}"
    assert abs(end2 - expected2[2]) <= 100, f"end {end2} vs expected {expected2[2]}; consensus padding: {cons2.consensus_padding}"

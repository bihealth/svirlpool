...
def test_trim_reads_multiple():
    ...
    with open(filepath, 'rb') as f:
        ...
        assert magic == b'BAM\x01' or magic[:3] == b'\x1f\x8b\x08', f"Invalid BAM header: {magic}"
...
def test_cut_reads_open_end_fwd():
    ...
    with tempfile.TemporaryDirectory() as tdir:
        ...
        reads = [ref[:6000] + util.generate_sequence(4000,seed=137),
                ref[:6000] + util.generate_sequence(2000,seed=138)]
        tmp_alignments = tempfile.NamedTemporaryFile(delete=True, suffix=".bam", dir=tdir)
        ...
        read_records=consensus.get_full_read_sequences_of_alignments(
            dict_alignments={0:{0:alns}},
            paths_alignments={0:tmp_alignments.name})
        cut_reads = consensus.trim_reads(
            dict_alignments={0:{0:alns}},
            intervals=max_intervals,
            read_records=read_records)
...
def test_cut_reads_open_end_rvs():
    with tempfile.TemporaryDirectory() as tdir:
        ref = util.generate_sequence(10_000,seed=31)
        reads = [util.reverse_complement(ref[6000:]) + util.generate_sequence(6000,137),
...
def test_get_ref_pitx_on_read__real_long_rev():
    ...
    for position,expected in zip(positions,expecteds):
        result = util.get_ref_pitx_on_read(
            alignment=aln,
            position=position,
            direction=util.Direction.NONE)[0]
        assert result == expected
...
def test_get_interval_on_read_in_region__real_long_rev():
    ...
    for region,expected in zip(regions,expectets):
...
def test_util_get_interval_on_read_in_region__simple():
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        interval = util.get_interval_on_read_in_region(a=alignment,start=region[0],end=region[1])
        # assert if interval is within 10 bp of expected_read_region
        assert expected_read_region[0]-10 <= interval[0] <= expected_read_region[0]+10
        assert expected_read_region[1]-10 <= interval[1] <= expected_read_region[1]+10
...
def test_util_get_interval_on_read_in_region__longer_clipped_right():
    ...
    read = reference.copy()[read_subsequence[0]:read_subsequence[1]]
    read.extend(util.generate_sequence(4000,seed=137)) # append more random DNA to the right
    expected_read_region = 1000,4000 # left is the position on the read, where the region starts, right is the min(region, read).
    # make temporary directory
    with tempfile.TemporaryDirectory() as tmpdirname:
        path_reference = Path(tmpdirname) / "reference.fasta"
        util.write_sequences_to_fasta([reference], path_reference, True)
...
def test_util_get_interval_on_read_in_region__longer_clipped_left():
    ...
    expected_read_region = 2000,5000 # left is the start of the read, right is the min(read,region).
    # make temporary directory
    with tempfile.TemporaryDirectory() as tmpdirname:
        path_reference = Path(tmpdirname) / "reference.fasta"
        util.write_sequences_to_fasta([reference], path_reference, True)
...
def test_get_ref_pitx_on_read_simple():
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        alignment = list(pysam.AlignmentFile(path_bam, "rb"))[0]
        # get interval on read
        positions_to_test = [1998,1999,2000,2001,  5999,6000,6001,6002] # positions on reference
...
def test_get_read_pitx_on_ref_simple():
    ...
    read = util.generate_sequence(1000,seed=1) + read + util.generate_sequence(1000,seed=2)
    # make temporary directory
    with tempfile.TemporaryDirectory() as tmpdirname:
        path_reference = Path(tmpdirname) / "reference.fasta"
        util.write_sequences_to_fasta([reference], path_reference, True)
        ...
        positions_to_test = [   1, 999,1000,1001,  4999,5000,5001,5999] # positions on read
        expected =          [2000,2000,2000,2001,  5999,6000,6000,6000] # positions on reference
        for i,pos in enumerate(positions_to_test):

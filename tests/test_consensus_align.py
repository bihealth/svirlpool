...
def test_process_partition_for_trf_overlaps_basic():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Core intervals: consensusID mapped to list of (chrom, start, end)
        core_intervals = {
            "consensus1": [
                ("chr1", 50, 150),   # Overlaps TRF at 100-200
                ("chr1", 1000, 2000) # Overlaps TRF at 1500-1600
            ],
            "consensus2": [
                ("chr1", 450, 650)   # Overlaps TRF at 500-600
            ],
            "consensus3": [
                ("chr1", 3000, 4000) # No overlaps
            ]
        }
...
def test_process_partition_for_trf_overlaps_multiple_overlaps():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        core_intervals = {
            "consensus1": [
                ("chr1", 50, 600)  # Overlaps all three TRFs
            ]
        }
...
def test_process_partition_for_trf_overlaps_multiple_chromosomes():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        core_intervals = {
            "consensus1": [
                ("chr1", 50, 150),   # Overlaps chr1 TRF
                ("chr2", 450, 650)   # Overlaps chr2 TRF
            ]
        }

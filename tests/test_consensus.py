...
from svirlpool.util.util import dict_to_seqrecord, seqrecord_to_dict
...
def test_create_padding_for_consensus() -> None:
    ...
    with gzip_open(
        DATA_DIR / "consensus_padding.1.json.gz", "rt"
    ) as f:
        data1 = json.load(f)
        ...
        cutreads1: dict[str, SeqRecord] = {
            name: dict_to_seqrecord(rec)
            for name, rec in data1["cutreads"].items()
        }
        read_records1: dict[str, SeqRecord] = {
            name: dict_to_seqrecord(rec)
            for name, rec in data1["read_records"].items()
        }
    with gzip_open(
        DATA_DIR / "consensus_padding.2.json.gz", "rt"
    ) as f:
        data2 = json.load(f)
        ...
        cutreads2: dict[str, SeqRecord] = {
            name: dict_to_seqrecord(rec)
            for name, rec in data2["cutreads"].items()
        }
        read_records2: dict[str, SeqRecord] = {
            name: dict_to_seqrecord(rec)
            for name, rec in data2["read_records"].items()
        }
    # generate results
    ...
    result_1 = consensus_class.ConsensusPadding(
        sequence='',
    ...
    result_2 = consensus_class.ConsensusPadding(
        sequence='',
